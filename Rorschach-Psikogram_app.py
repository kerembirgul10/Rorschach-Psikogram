...
def analysis_form(edit_data=None):
    # Form yapısı hız sağlar ancak tıklanabilir kutular için State kullanmalıyız
    ...
    h_isim = c_info1.text_input("Hastanın Adı Soyadı", value=d_name)
    h_yas = c_info2.number_input("Yaş", 0, 120, value=d_age)
    ...
    def box_selector(label, key_prefix, saved_val):
        ...
        if f"{key_prefix}_list" not in st.session_state:
            st.session_state[f"{key_prefix}_list"] = saved_list
    ...
    w_cards = box_selector("En Beğenmedigi Kartlar", "worst", edit_data.get('en_beğenmediği', "[]") if edit_data else "[]")
...
if not st.session_state['logged_in']:
    ...
    if menu == "Hastalarım":
        ...
        search = st.text_input("Hasta Ara...", placeholder="İsim yazın...")
        ...
        if not my_p.empty:
            ...
            for _, row in filtered.iterrows():
                if st.button(row['hasta_adi'], key=f"p_{_}"):
